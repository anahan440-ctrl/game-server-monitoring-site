...
def handler(event: dict, context) -> dict:
    ...
    if method == "GET":
        ...
        if resource == "updates":
            ...
            if game:
                cur.execute(
                    f"SELECT id, game, name, map, ip, max_players, is_active FROM {schema}.servers WHERE game=%s AND is_active=TRUE ORDER BY id",
                    (game,)
                )
            else:
                cur.execute(
                    f"SELECT id, game, name, map, ip, max_players, is_active FROM {schema}.servers WHERE is_active=TRUE ORDER BY id"
                )
            rows = cur.fetchall()
            conn.close()
            data = [
    ...
    if resource == "servers":
        if method == "POST":
            ...
            cur = conn.cursor()
            cur.execute(
                f"INSERT INTO {schema}.servers (game, name, map, ip, max_players) VALUES (%s,%s,%s,%s,%s) RETURNING id",
                (body["game"], body["name"], body["map"], body.get("ip",""), int(body.get("max_players", 60)))
            )
        ...
        if method == "PUT":
            ...
            cur = conn.cursor()
            cur.execute(
                f"UPDATE {schema}.servers SET game=%s,name=%s,map=%s,ip=%s,max_players=%s,is_active=%s WHERE id=%s",
                (body["game"], body["name"], body["map"], body.get("ip",""), int(body.get("max_players",60)), bool(body.get("is_active",True)), body["id"])
            )
            conn.commit()
            conn.close()
